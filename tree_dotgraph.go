...
func WriteDOTGraph(w io.Writer, tree *ImmutableTree, paths []PathToLeaf) {
	...
	tree.root.traverse(tree, true, func(node ComplexNode) bool {
		...
		graphNode.Label = mkLabel(string(node.GetKey()), 16, "sans-serif")
		...
		if node.GetValue() != nil {
			graphNode.Label += mkLabel(string(node.GetValue()), 10, "sans-serif")
		}
		...
		if node.LeftNode() != nil {
			ctx.Edges = append(ctx.Edges, &graphEdge{
				From: graphNode.Hash,
				To:   fmt.Sprintf("%x", node.LeftNode().getHash()),
			})
		}
		if node.RightNode() != nil {
			ctx.Edges = append(ctx.Edges, &graphEdge{
				From: graphNode.Hash,
				To:   fmt.Sprintf("%x", node.RightNode().getHash()),
			})
		}
		return false
	})
